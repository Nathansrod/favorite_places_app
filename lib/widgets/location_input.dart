...
class LocationInput extends StatefulWidget {
  ...
  final void Function(FavoritePlaceLocation location) onSelectLocation;
...
class _LocationInputState extends State<LocationInput> {
  FavoritePlaceLocation? _pickedLocation;
  ...
  Future<void> _saveFavoritePlace(double latitude, double longitude) async {
    final url = Uri.parse(
        'https://maps.googleapis.com/maps/api/geocode/json?latlng=$latitude,$longitude&key=$kMapsApiKey-6irONc');
    final response = await http.get(url);
    final resData = json.decode(response.body);
    final address = resData['results'][0]['formatted_address'];

    setState(() {
      _pickedLocation = FavoritePlaceLocation(
        latitude: latitude,
        longitude: longitude,
        address: address,
      );
